...
jQuery(document).ready(function () {
  ...
  runOnUserContent(function () {
    ...
    ).prepend('<span class="dynamic"></span>');
    jQuery(
      ".context-course_11 #question_481_question_text input, .context-course_15 #question_618_question_text input, .context-course_23 #question_1917_question_text input, .context-course_25 #question_2259_question_text input"
    ).each(function (i) {
    ...
    jQuery("#bz-auto-toc").each(function () {
      bzAutoTOC();
    });
    ...
    function addIndex() {
      ...
      $(
        'h2:not(".box-title"),h3:not(".box-title"),h4:not(".box-title")',
        $(".bz-module:not(.lc-module)")
      ).each(function (i, e) {
    ...
    jQuery(".bz-app-ritual tbody").on(
      "change",
      ".bz-app-ritual-my-week-value",
      function (e) {
        ...
        $modules.each(function (i, week) {
          ...
          BZ_SaveMagicField(
            $semesterInput.attr("data-bz-retained"),
            $semesterInput.val()
          );
          if ($weekValue.val() == "") {
            $goalInput.val("");
    ...
    jQuery(".bz-app-ritual-score").after(
      '<span class="bz-app-ritual-score-text">40</span>'
    );
    ...
    jQuery(".modal").on("change", ".slider", function (e) {
      ...
      BZ_SaveMagicField(
        $totalScore.attr("data-bz-retained"),
        $totalScore.val()
      );
    });
    ...
    jQuery(".bz-app-ritual-scorecard-apply").on("click", function () {
      ...
      BZ_SaveMagicField(
        $scoreField.attr("data-bz-retained"),
        $scoreField.val()
      );
      jQuery(".bz-app-ritual .modal").hide();
    });
    ...
    jQuery(".bz-app-ritual-applications").on(
      "keydown",
    ...
    addOnMagicFieldsLoaded(function () {
      jQuery("input.bz-app-ritual-check")
        .not('[value=""]')
        .each(function (i, c) {
          $c = jQuery(c);
          if ($c.val() == "✓") {
...
var bzNewUiHandlers = {
  ...
  '[data-grade-as="checklist"] .done-button': function () {
    ...
    checklist.children().each(function () {
      ...
      if ($option.children("input").is(":checked")) {
        ...
        if (
          $option.find("input").is('[data-correctness="correct"]') ||
          $option.parents(".module-checkbox-div").is(".correct")
        ) {
          checklistScore++;
        } else {
          falsePositives++;
    ...
    var feedback =
      "You got " + checklistScore + " out of " + maxScore + " right";
    if (0 < falsePositives) {
  ...
  '[data-grade-as="radio"] .done-button': function () {
    ...
    radiolist.children().each(function () {
      ...
      if ($option.children("input").is(":checked")) {
        ...
        if (
          $option.find("input").is('[data-correctness="correct"]') ||
          $option.parents(".module-radio-div").is(".correct")
        ) {
          feedback = "Good job!";
          feedbackClass = "correct";
        } else {
  ...
  '[data-grade-as="range"] .done-button': function () {
    ...
    var rangeScore =
      max - (max * Math.abs(correctVal - currentVal)) / (max - min);
    if (max <= rangeScore) {
  ...
  '[data-grade-as="matching"] .done-button': function () {
    jQuery(this)
      .parents(".module-block")
      .find(".sort-to-match")
      .addClass("show-answers");
  },
  ...
  ".for-eval-sum": function () {
    ...
    var max =
      10 * jQuery(this).parents(".content-section").find(".criterion").length;
    // return results
    jQuery(this)
      .parents(".content-section")
      .next()
      .find(".bz-show-eval-sum")
      .text(String(results));
    jQuery(this)
      .parents(".content-section")
      .next()
      .find(".bz-show-eval-max")
      .text(String(max));
  },
...
function shuffleChildren(element) {
  ...
  for (var i = 0; i < shuffled.length; i++) element.appendChild(shuffled[i]);
...
function matchesSelector(element, selector) {
  if (element.matches) return element.matches(selector);
    ...
    for (var i = 0; i < all.length; i++) if (all[i] == element) return true;
    return false;
  }
}
...
function currentRangeVal(e) {
  ...
  jQuery(e)
    .parents(".question")
    .find(".display-value .current-value")
    .text(currentVal);
...
function bzInitializeNewUi() {
  ...
  jQuery(".bz-check-answers")
    .parents(".module-block")
    .find(".box-title")
    .next()
    .addClass("bz-graded-question");
  ...
  jQuery('[data-instant-feedback="true"]')
    .find("input")
    .change(function () {
      var answerParent = jQuery(this).parents(
        "td,.module-checkbox-div,.module-radio-div"
      );
  ...
  */
  ...
  jQuery("fieldset")
    ...
    .each(function () {
      ...
      var validChildren = fieldset.children(
        ".module-checkbox-div, .module-radio-div"
      );
...
runOnUserContent(function () {
  ...
  jQuery(".bz-module:not(.lc-module)").prepend(function () {
    var tocBox = jQuery(
      '<div class="main-toc"><p class="match-heading-style">The big picture:</p></div>'
    );
  ...
  jQuery(".checklist-other")
    .bind("keyup blur", function (e) {
      jQuery(this)
        .siblings('[type="checkbox"], [type="radio"]')
        .prop("checked", jQuery(this).val() ? true : false);
    })
    .parent("p")
    .addClass("bz-has-other");
  ...
  jQuery(".conditional-show-source")
    .find("input")
    .each(function () {
  ...
  function showIfMagicIsPopulated(magicInput) {
    addOnMagicFieldsLoaded(function () {
      jQuery(
        ".conditional-show [data-bz-retained=" +
          magicInput.data("bz-retained") +
          '], [data-bz-reference="' +
          magicInput.data("bz-retained") +
          '"]'
      ).each(function () {
        if (
          magicInput.prop("checked") ||
          (!magicInput.is('[type="checkbox"], [type="radio"]') &&
            magicInput.val() !== "")
        ) {
  ...
  function setupBTT() {
    ...
    var btt = jQuery(
      '<span role="button" tabindex="0" id="bz-back-to-top" class="match-heading-style">Back to top</span>'
    ).click(function () {
...
function bzAutoTOC() {
  ...
  $.subscribe("userContent/change", function () {
    bzAutoTOCImpl();
  }); // and schedule a call in the future
}
...
function bzAutoTOCImpl() {
  ...
  if (toc.className == "bz-already-loaded") return;
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    if (mid == item.id) li.className = "bz-toc-current";
    var a = document.createElement("a");
    a.textContent = item.title;
    a.href = item.url
      ? item.url
      : "/courses/" + item.context_id + "/modules/items/" + item.id;
    li.appendChild(a);
    ol.appendChild(li);
  }
  toc.appendChild(ol);
...
function bzAfterLL() {
  jQuery("#context_modules .context_module_item").each(function () {
    if (
      jQuery(this).text().toLowerCase().indexOf("after learning lab") > -1 ||
      jQuery(this).text().toLowerCase().indexOf("take a break") > -1
    ) {
...
function bzPageMapperPageCharCount() {
  ...
  jQuery(".bz-pm-pl").each(function () {
    ...
    jQuery(this)
      .css("background", "rgba(255,0,0," + (pageLength / avgLength - 1) + ")")
      .attr({
...
function bzLocalNavUI() {
  jQuery("#bz-module-nav .has-children")
    .not("#bz-module-nav > li")
    .append('<div class="bz-nav-ui">+</div>');
  jQuery(".bz-nav-ui")
    .addClass(function () {
      // add a class based on whether this is already expanded or collapsed
      if (
        jQuery(this).siblings(".children").children("li").css("display") ==
        "none"
      ) {
        return "collapsed";
      } else {
  ...
  jQuery("#bz-module-nav ul.active-parent")
    .parent()
    .siblings("li")
    .addClass("active-uncles")
    .show();
}
...
*/
function bzReplaceInlineRubrics(
  queuedInlineRubricsToReplace,
  loadedFullRubricTables,
  loadedMagicFieldValues
) {
  while (queuedInlineRubricsToReplace.length > 0) {
    ...
    console.log(
      'Replacing <a class="bz-ajax-replace" href="' +
        rubricURL +
        criterion_selector +
        '">' +
        el.html() +
        "</a> with inline rubric"
    );
    ...
    if (!fullRubricTable) {
      ...
      console.log(
        "Error: replacing rubric. It hasnt been fetched yet. Maybe there is a mix and match of rubrics on this page?"
      );
      continue;
    }
    ...
    ).html(fullRubricTable.find(criterion_selector));
    ...
    if (magicFieldName) {
      ...
      if (ratingsTable) {
        ...
        ratingsTable.on("click", "td", function () {
          ...
          BZ_SaveMagicField(
            magicFieldName,
            selectedvalue,
            isOptionalMagicField,
            "hidden"
          );
        });
      } else {
        console.log(
          'Error: Failed finding <table class="ratings"/> for magic rubric: ' +
            magicFieldName
        );
      }
      ...
      if (obj && obj.value && obj.value != "undefined") {
        // On first load, there may not be a value set yet.
        ...
        if (selectedCell) selectedCell.addClass("selected");
          ...
          console.log(
            "Error: Failed finding td#" +
              magicValue +
              " to set as selected cell for magic rubric"
          );
      }
    }
...
function bzAjaxLoad() {
  ...
  jQuery(".bz-ajax-replace").each(function (e) {
    ...
    if (replaceURL.indexOf("rubric") >= 0) {
      ...
      if (
        !queuedFullRubricTables.has(rubricURL) &&
        !loadedFullRubricTables.hasOwnProperty(rubricURL)
      ) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            if (queuedExistingRubricValues.length <= 0) {
              bzReplaceInlineRubrics(
                queuedInlineRubricsToReplace,
                loadedFullRubricTables,
                loadedExistingRubricValues
              );
            }
          },
        });
      ...
      if (ENV["WIKI_PAGE"] || ENV["ASSIGNMENT_ID"]) {
        this.href = replaceURL.replace(
          /RETURN_TO_TOKEN/,
          encodeURIComponent(currentPath)
        );
        console.log(
          "Dynamically replaced RETURN_TO_TOKEN in this LinkedIn Authorize button URL: " +
            replaceURL +
            " with: " +
            this.href
        );
        ...
        console.log(
          "LinkedIn API authorization only works on assignments and wiki pages.  Returning."
        );
        return;
      }
      ...
      if (ENV["WIKI_PAGE"] || ENV["ASSIGNMENT_ID"]) {
        ...
        if (dataApiEndpointReplace) {
          jQuery(this).attr(
            "data-api-endpoint",
            dataApiEndpointReplace.replace(/LOCAL_COURSE_ID_TOKEN/, course_id)
          );
        }
        console.log(
          "Fixed the following link to point to the local course: " + this.href
        );
      } else {
        console.log(
          "Fixing up Content Library links only works for pages and assignments. Returning."
        );
        return;
      }
      jQuery(this).removeClass("bz-ajax-replace");
  ...
  jQuery('a[href*="/oauth?service=linked_in&return_to=RETURN_TO_TOKEN"]').each(
    function (e) {
      var el = jQuery(this);
      let replaceURL = jQuery(this).attr("href");
  ...
  jQuery
    .get("/bz/user_linkedin_url?" + Math.random())
    .success(function (ajaxResponse) {
      var evt = document.createEvent("HTMLEvents");
      evt.initEvent("change", true, true);
      jQuery(".bz-user-linkedin-profile")
        .val(ajaxResponse.linkedin_url)[0]
        .dispatchEvent(evt);
    });

  // If there are clickable rubrics, go load their clicked values.
  if (queuedExistingRubricValues.length > 0) {
    BZ_LoadMagicFields(queuedExistingRubricValues, function (
      retrievedFieldValues
    ) {
...
function createBzProgressBar() {
  ...
  function setParticipationScore(score) {
    participationScore.textContent =
      "Score so far: " + Math.round(100 * score) / 100 + " / 10";
    participationScoreContext.textContent =
      "This score is a combination of how you do on Mastery Questions and your full participation in the module.";
  }
  ...
  function redrawMastery() {
    ...
    for (var i = 0; i < possibilities.length; i++) {
      ...
      if (e.matches(".done-button")) boxPosition++;
      ...
      if (e.dataset.bzRetained == lastName) continue;
      ...
      if (boxPosition < window.openBzBoxPosition) {
        ...
        if (
          ...
          (e.type == "checkbox" &&
            e.getAttribute("data-bz-answer") == "yes" &&
            e.checked) ||
          // in some cases we used yes, in some we used on, allow either as correct
          (e.type == "checkbox" &&
            e.getAttribute("data-bz-answer") == "on" &&
            e.checked) ||
          (e.type == "checkbox" &&
            e.getAttribute("data-bz-answer") == "" &&
            !e.checked) ||
          (e.type != "radio" &&
            e.type != "checkbox" &&
            e.value == e.getAttribute("data-bz-answer"))
        ) {
          correct = true;
        } else {
          correct = false;
...
runOnUserContent(function () {
  ...
  var position_magic_field_name = window.position_magic_field_name
    ? window.position_magic_field_name
    : "module_position_" + ENV["WIKI_PAGE"].page_id;
  ...
  */
  ...
  function unhideNext(obj) {
    ...
    for (var i = 0; i < n.length; i++) {
      ...
      if (n[i].querySelector(".done-button")) break;
    }
  }
  ...
  if (allBoxesWithStoppingPoints.length == 0 || isNaN(openPosition)) return;
  ...
  var first =
    openPosition == allBoxesWithStoppingPoints.length
      ? null
      : allBoxesWithStoppingPoints[openPosition].querySelector(".done-button");
  var listOfShowingBoxes =
    first == null ? allBoxesWithStoppingPoints : collectBoxesBeforeBox(first);
  ...
  addOnMagicFieldsLoaded(function () {
    ...
    for (var i = 0; i < list.length; i++) {
      if (list[i].classList.contains("module-block")) {
        ...
        if (button) triggerBzNewUiHandler(button);
  ...
  jQuery(".done-button")
    .click(function (e) {
      ...
      if (
        parentBox.querySelector('[data-instant-feedback="true"]') &&
        !parentBox.querySelector(".show-answers")
      ) {
      ...
      if (pos < openPosition) return; // no need to update if they clicked done again on a previous button; keep them at the advanced position
      ...
      var data =
        "optional=1&name=" +
        encodeURIComponent(position_magic_field_name) +
        "&value=" +
    ...
    .parents(".module-block")
    .addClass("bz-has-toggle-btn");

  if (location.hash.length) {
    var where = document.getElementById(location.hash.substring(1));
    if (where) where.scrollIntoView();
  } else if (openPosition != 0 && openPosition < allBoxesWithStoppingPoints.length && allBoxesWithStoppingPoints.length > 0) allBoxesWithStoppingPoints[openPosition].scrollIntoView();
});
...
runOnUserContent(function () {
  ...
  function sortToMatchSetup() {
    ...
    function isValidDropTarget(event, dropping) {
      ...
      if (
        dragging.getAttribute("data-column-number") ==
        dropping.getAttribute("data-column-number")
      )
        return true;
      return false;
    }
    ...
    var sortToMatch = document.querySelectorAll(
      ".sort-to-match td:not(:first-child)"
    );
    for (var i = 0; i < sortToMatch.length; i++) {
      var td = sortToMatch[i];

      var p = td.parentNode;
      while (p.tagName != "TABLE") p = p.parentNode;
      ...
      function changeDropTarget(to) {
        if (dropTargetViaKeyboard) {
          ...
          dropTargetViaKeyboard.className = dropTargetViaKeyboard.className.replace(
            " inside-dragging",
            ""
          );
        }
      ...
      wrapper.addEventListener("keydown", function (event) {
        ...
        switch (event.keyCode || event.which) {
          ...
          case 38: // up arrow
            if (event.target.getAttribute("aria-grabbed") == "true") {
              ...
              iterateDropTarget(
                "TABLE",
                '.droppable[data-column-number="' +
                  dropTargetViaKeyboard.getAttribute("data-column-number") +
                  '"]',
                -1
              );
            }
        ...
        function iterateDropTarget(parentTag, selector, change) {
          ...
          if (!tbl) return;
          while (tbl.tagName != parentTag) tbl = tbl.parentNode;
          var allTgts = tbl.querySelectorAll(selector);
          for (var loopVar = 0; loopVar < allTgts.length; loopVar++)
            if (allTgts[loopVar] == dropTargetViaKeyboard) break;
          loopVar += change;
          // wraparound behavior
          if (loopVar < 0) loopVar += allTgts.length;
          else if (loopVar >= allTgts.length) loopVar -= allTgts.length;
          ...
          if (rect.top < 0 || rect.bottom + 40 > window.innerHeight) {
            ...
            if (change > 0) window.scrollBy(0, 40); // scroll a bit more when doing down so the "Back to Top" doesn't overlap any text
          }
        }
      });
      ...
      td.addEventListener("drop", function (event) {
        ...
        var dragging;
        try {
          dragging = document.getElementById(
    ...
    for (var i = 0; i < sortToMatch.length; i++) {
      ...
      while (parentBox) {
        if (parentBox.classList.contains("module-block")) break;
        parentBox = parentBox.parentNode;
      }
      ...
      for (var col = 1; col < columns.length; col++) {
        ...
        var draggablesDom = table.querySelectorAll(
          "td:nth-child(" + (col + 1) + ").droppable > [draggable]"
        );
        var droppables = table.querySelectorAll(
          "td:nth-child(" + (col + 1) + ").droppable"
        );
        ...
        for (var a = 0; a < shuffled.length; a++) {
          if (
            shuffled[a].id.replace("draggable", "droppable") == droppables[a].id
          ) {
  ...
  function sortToMatchCheck(sortToMatchTable, updateMagicField) {
    ...
    for (var row = 0; row < rows.length; row++) {
      ...
      for (var a = 0; a < all.length; a++) {
        ...
        var magicFieldName = String.fromCharCode(
          65 + Number(d.getAttribute("data-table-cell-id"))
        );
        magicFieldSequence += magicFieldName;

        var thisOneCorrect =
          d.parentNode.id == d.id.replace("draggable", "droppable");
      ...
      if (all.length) tr.className = allCorrect ? "correct" : "incorrect";
    }
...
if (0)
  runOnUserContent(function () {
    ...
    function doSomething(scroll_pos) {
      ...
      if (start >= sections.length) return;
